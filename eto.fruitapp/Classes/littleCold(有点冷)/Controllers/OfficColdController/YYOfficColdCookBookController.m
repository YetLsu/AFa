//
//  YYOfficColdCookBookController.m
//  eto.fruitapp
//
...
@interface YYOfficColdCookBookController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation YYOfficColdCookBookController
...
//获取所有文章
- (void)getAllArticles{
    NSString *urlStr = @"http://www.sxeto.com/fruitApp/Buyer?mode=31&category=4";
...
- (instancetype)initWithTableViewHeight:(CGFloat)height{
    if (self = [super init]) {
        ...
        self.cookBookTableView = tableView;
        self.cookBookTableView.delegate = self;
        self.cookBookTableView.dataSource = self;
        [self getAllArticles];
        //self.cookBookTableView.backgroundColor = [UIColor redColor];
