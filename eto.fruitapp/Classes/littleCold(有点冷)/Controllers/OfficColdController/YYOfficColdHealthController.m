...
@interface YYOfficColdHealthController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation YYOfficColdHealthController
...
//获取所有文章
- (void)getAllArticles{
    NSString *urlStr = @"http://www.sxeto.com/fruitApp/Buyer?mode=31&category=3";
...
- (instancetype)initWithTableViewHeight:(CGFloat)height{
    if (self = [super init]) {
        ...
        self.healthTableView = tableView;
        self.healthTableView.delegate = self;
        self.healthTableView.dataSource = self;
        [self getAllArticles];
        //self.healthTableView.backgroundColor = [UIColor purpleColor];
